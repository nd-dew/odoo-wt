...
{args_block_str}
        ...
        for arg in args:
            if " " in arg or "," in arg:
                bash_args.append(f"'{arg}'")
            else:
                bash_args.append(arg)

        run_script_content = f"""#!/bin/bash
# Clean, modern run script automatically generated by odoo-wt

# Wait 1.5 seconds in the background for Odoo's HTTP server to bind, then cleanly open your browser!
(
    sleep 1.5
...
# Execute Odoo server using uv run!
exec uv run {self.comm_dir}/odoo-bin {' '.join(bash_args)} "$@"
"""
        try:
